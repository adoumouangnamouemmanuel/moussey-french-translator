const genesisChapter45 = {
  "1-45": [
    {
      id: "1-45-1",
      verse: 1,
      text: "Yosef ka ndak a doo tam ii suu laki egemma oloɗi. Nam ɓak kolo eŋga ana cuk suuna ki kiɗik halaŋ. Nam tak tam ii gorsemma na ni sa hin ka usi kalafi dew oloɗi.",
      french:
        "Joseph ne pouvait plus se contenir devant tous ceux qui l'entouraient. Il s'écria: Faites sortir tout le monde. Et il ne resta personne avec Joseph, quand il se fit connaître à ses frères.",
    },
    {
      id: "1-45-2",
      verse: 2,
      text: "Nam tli delem kolo tiina, suu Egiptena hum ha ki lay, suu ha vo vi Farawna hum ha ki lay.",
      french:
        "Il éleva la voix, en pleurant. Les Égyptiens l'entendirent, et la maison de Pharaon l'entendit.",
    },
    {
      id: "1-45-3",
      verse: 3,
      text: "Yosef di gorsemma ana, «Ni an Yosef. Bun ka iiri tu su?» Gorsemma ka ndak a hoŋom dira oloɗi kay asi wam na ni mbuɗ ki ndaa cocoo.",
      french:
        "Joseph dit à ses frères: Je suis Joseph! Mon père vit-il encore? Mais ses frères ne purent lui répondre, car ils étaient troublés en sa présence.",
    },
    {
      id: "1-45-4",
      verse: 4,
      text: "Yosef dasi ana, «Ҥuɗ tagi ay egenu.» Asi ɦuɗ tasi aya. Nam dasi ana, «Ni an wayagina, Yosef, ma agi gusum ki ko suu Egiptena.",
      french:
        "Joseph dit à ses frères: Approchez-vous de moi. Et ils s'approchèrent. Il dit: Je suis Joseph, votre frère, que vous avez vendu pour être mené en Égypte.",
    },
    {
      id: "1-45-5",
      verse: 5,
      text: "Cemi wanda yoorogi tagiɗi, hurugi haɗ ana agi gusun ay ki mbaaran ay ki taniraɗi, Lona sunun ay ki fokogi ni a suɗugi iiri magira.",
      french:
        "Maintenant, ne vous affligez pas, et ne soyez pas fâchés de m'avoir vendu pour être conduit ici, car c'est pour vous sauver la vie que Dieu m'a envoyé devant vous.",
    },
    {
      id: "1-45-6",
      verse: 6,
      text: "Mayra li basara ki ca ni mba', hin fok basara vadl ni sa ka horokgi lay, ka feɗ waɗi lay.",
      french:
        "Voilà deux ans que la famine est dans le pays; et pendant cinq années encore, il n'y aura ni labour, ni moisson.",
    },
    {
      id: "1-45-7",
      verse: 7,
      text: "Lona sunun ay ki fokogi ni a dokogi njafagi kay ndaŋgara, nam suɗugi iiri magira kini suɗta ŋgolla.",
      french:
        "Dieu m'a envoyé devant vous pour vous faire subsister dans le pays, et pour vous faire vivre par une grande délivrance.",
    },
    {
      id: "1-45-8",
      verse: 8,
      text: "Agi sunun ay ka tani ni ka agiɗi, sunun ay ni Lona, nam tinin a sa ŋgolna vi Faraw, dlara hu ziimba halaŋŋi konu. Anni a mulna kay mbassa Egiptera halaŋ.",
      french:
        "Ce n'est donc pas vous qui m'avez envoyé ici, mais c'est Dieu; il m'a établi père de Pharaon, maître de toute sa maison, et gouverneur de tout le pays d'Égypte.",
    },
    {
      id: "1-45-9",
      verse: 9,
      text: "Ɓaɗagi zak hoŋogi eg bunu, agi dam ana gooŋŋa Yosef ɓak ay ana, Lona tinin a ŋgolla hu mbassa Egiptera, aŋ mbay egen tani vi yaŋ ayɗi.",
      french:
        "Hâtez-vous de remonter auprès de mon père, et vous lui direz: Ainsi a parlé ton fils Joseph: Dieu m'a établi seigneur de toute l'Égypte; descends vers moi, ne tarde pas!",
    },
    {
      id: "1-45-10",
      verse: 10,
      text: "Aŋ hin mba cukŋi hu mbassa Gosenda egen tani na tok, aŋ taŋga, goryoŋu, u gor ŋgologi maŋsina u tlegeyeŋ halaŋ.",
      french:
        "Tu habiteras dans le pays de Gosen, et tu seras près de moi, toi, tes fils, et les fils de tes fils, tes brebis et tes boeufs, et tout ce qui est à toi.",
    },
    {
      id: "1-45-11",
      verse: 11,
      text: "Ni an hin ɦaŋ tina u yoori maŋsina halaŋ. Kay mayra hin a cini basara vadl tuwa. Ni va hin ka laŋgi, aŋ u suu maŋsina halaŋ, u tlegeyeŋ lay.",
      french:
        "Là, je te nourrirai, car il y aura encore cinq années de famine; et ainsi tu ne périras point, toi, ta maison, et tout ce qui est à toi.",
    },
    {
      id: "1-45-12",
      verse: 12,
      text: "Agi wanni u tagira lay, wayanna Benjamin wan u irim lay. Ɓakagi ni an tanda Yosef daŋ may.",
      french:
        "Vous voyez de vos yeux, et mon frère Benjamin voit de ses yeux que c'est moi-même qui vous parle.",
    },
    {
      id: "1-45-13",
      verse: 13,
      text: "Cuɗugi dla ŋgol manda hu Egiptera ki maŋ bun halaŇ u tlesuu agi wasi u irigina lay. Agi hin mbaaragi bun ay tani na zak.»",
      french:
        "Racontez à mon père toute ma gloire en Égypte, et tout ce que vous avez vu; et vous ferez descendre ici mon père au plus tôt.",
    },
    {
      id: "1-45-14",
      verse: 14,
      text: "Nam mboɗ wayamma Benjamin hurum mboɗni tiira, Benjamin tii may.",
      french:
        "Il se jeta au cou de Benjamin, son frère, et pleura; et Benjamin pleura sur son cou.",
    },
    {
      id: "1-45-15",
      verse: 15,
      text: "Nam furi gorsem suu hiŋŋa halaŋ u tiina. Ŋgoo dagan gorsemma lak njaɗ u namu.",
      french:
        "Il embrassa aussi tous ses frères, en pleurant. Après quoi, ses frères s'entretinrent avec lui.",
    },
    {
      id: "1-45-16",
      verse: 16,
      text: "Vunna kal ha ki vi Faraw ana, «Gorseŋ Yosefna mba wa.» Boy namma jivi ii Faraw u suu mam suu ŋgolona cocoo.",
      french:
        "Le bruit se répandit dans la maison de Pharaon que les frères de Joseph étaient arrivés: ce qui fut agréable à Pharaon et à ses serviteurs.",
    },
    {
      id: "1-45-17",
      verse: 17,
      text: "Faraw di Yosef ana, «Di gorseŋŋa asi li ko wanda na, yowogi tlegeyegi kay korro magira, tuɗugi vo Kanaan.",
      french:
        "Pharaon dit à Joseph: Dis à tes frères: Faites ceci. Chargez vos bêtes, et partez pour le pays de Canaan;",
    },
    {
      id: "1-45-18",
      verse: 18,
      text: "Yowogi bugi ay u suu magina halaŋ mbaagi ay egen tani. An hin ɦagi li ma jivi hu Egiptena ni magiya, agi hin tagi mbul ndaŋgara ni agiya.",
      french:
        "prenez votre père et vos familles, et venez auprès de moi. Je vous donnerai ce qu'il y a de meilleur au pays d'Égypte, et vous mangerez la graisse du pays.",
    },
    {
      id: "1-45-19",
      verse: 19,
      text: "Aŋ gaɗasi ana asi li ko wanda naa, yowogi kulumi suu pus pusna yow goryogi u boyogogi aya, yow bugi ay mberegi ay tani.",
      french:
        "Tu as ordre de leur dire: Faites ceci. Prenez dans le pays d'Égypte des chars pour vos enfants et pour vos femmes; amenez votre père, et venez.",
    },
    {
      id: "1-45-20",
      verse: 20,
      text: "Saaragi kay tlesuu agi hinisi ha kinaɗi, kay tlesuu jivi suu hu mbassa Egiptera ni magiya.»",
      french:
        "Ne regrettez point ce que vous laisserez, car ce qu'il y a de meilleur dans tout le pays d'Égypte sera pour vous.",
    },
    {
      id: "1-45-21",
      verse: 21,
      text: "Goryoŋ Israelna li ko ndaɗta na may. Yosef ɦasi kulumi suu pus pusna ko Faraw dira na may, nam ɦasi tlesuu ti ha voɗna may.",
      french:
        "Les fils d'Israël firent ainsi. Joseph leur donna des chars, selon l'ordre de Pharaon; il leur donna aussi des provisions pour la route.",
    },
    {
      id: "1-45-22",
      verse: 22,
      text: "Nam ɦasi baraw ma cukka tasi dew dew halaŋ. Benjamin nam ɦam gursu silira kis kis hindi, ɦam barawna ii cukka vadl may.",
      french:
        "Il leur donna à tous des vêtements de rechange, et il donna à Benjamin trois cents sicles d'argent et cinq vêtements de rechange.",
    },
    {
      id: "1-45-23",
      verse: 23,
      text: "Nam sun korra ha doogo u tlesuu jivi suu hu Egiptena ha maŋ bumu, korro hiŋŋa doogo u wana u tlesuu tira suu bum hin tasi ay irim voɗna may.",
      french:
        "Il envoya à son père dix ânes chargés de ce qu'il y avait de meilleur en Égypte, et dix ânesses chargées de blé, de pain et de vivres, pour son père pendant le voyage.",
    },
    {
      id: "1-44-24",
      verse: 24,
      text: "Nam hin gorsemma tuɗa, gaɗasi ana, «Lagi honi ha voɗɗi.»",
      french:
        "Puis il congédia ses frères, qui partirent; et il leur dit: Ne vous querellez pas en chemin.",
    },
    {
      id: "1-45-25",
      verse: 25,
      text: "Asi yow ay Egipte tew vo hu mbassa Kanaanda eg busi Yakob.",
      french:
        "Ils remontèrent de l'Égypte, et ils arrivèrent dans le pays de Canaan, auprès de Jacob, leur père.",
    },
    {
      id: "1-45-26",
      verse: 26,
      text: "Asi dam ana, «Yosef ka iiri tuwa, nam a ŋgolla hu mbassa Egiptera ni namu.» Yakob seŋ tam di va kaɗi, kay nam vi ka u boy masina tuɗi.",
      french:
        "Ils lui dirent: Joseph vit encore, et même c'est lui qui gouverne tout le pays d'Égypte. Mais le cœur de Jacob resta froid, parce qu'il ne les croyait pas.",
    },
    {
      id: "1-45-27",
      verse: 27,
      text: "Asi cuɗum boy ma Yosef dasina ayna halaŋ. Nam wi kulumi suu pus pusna suu Yosef sunusi ay kay a yowom hana na ni, gijaŋ a vasi boy masina.",
      french:
        "Ils lui rapportèrent toutes les paroles que Joseph leur avait dites. Il vit les chars que Joseph avait envoyés pour le transporter. C'est alors que l'esprit de Jacob, leur père, se ranima;",
    },
    {
      id: "1-45-28",
      verse: 28,
      text: "Israel di ana, «Ndak wa na ga, goonna Yosef ka iiriya, an tuɗ wam ki jew u iiri tan gaw.»",
      french:
        "et Israël dit: C'est assez! Joseph, mon fils, vit encore! J'irai, et je le verrai avant que je meure.",
    },
  ],
};

export default genesisChapter45;